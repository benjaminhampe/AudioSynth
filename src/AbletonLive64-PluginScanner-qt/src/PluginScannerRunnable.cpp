...
// static
VstIntPtr
vstHostCallbackDummy_static( AEffect* effect, VstInt32 opcode, VstInt32 index, VstIntPtr value, void *ptr, float opt )
{
   switch( opcode )
   {
...
intptr_t
vstDispatch( AEffect* vst, int32_t opcode, int32_t index = 0, intptr_t value = 0, void* ptr = nullptr, float opt = 0.0f )
{
   if ( !vst ) { return 0; }
...
PluginScannerRunnable::PluginScannerRunnable( PluginScanner* scanner, std::wstring const & uri )
   : m_scanner( scanner )
   , m_uri( uri )
{}
PluginScannerRunnable::~PluginScannerRunnable()
{}

void
PluginScannerRunnable::run()
{
   if ( m_uri.empty() )
   {
      DE_DEBUG( "Thread[",QThread::currentThreadId(),"] Got empty uri" )
      return;
   }

   DE_DEBUG( "Thread[",QThread::currentThreadId(),"] Scan DLL file ", dbStr(m_uri) )

   de::audio::PluginInfo pin;
   pin.m_uri = m_uri;
   pin.m_uriMB = toMultiByte( m_uri );
   pin.m_nameMB = de::FileSystem::fileBase( pin.m_uriMB );
   dbStrReplace( pin.m_nameMB, "&apos;", "'" );
   pin.m_name = QString::fromStdString( pin.m_nameMB ).toStdWString();
   pin.m_isFile = false;
...
//   }
   ...
   HMODULE dll = LoadLibrary( m_uri.c_str() );
   if ( !dll )
   {
      std::wcout << "Can't open dll, probably not in 64 bit." << m_uri << std::endl;
      m_scanner->addBadPluginInfo( pin );
      return;
   }
   ...
   if ( !fn_entryProc )
   {
      std::wcout << "No entry VSTPluginMain(), fallback to main... " << m_uri << std::endl;
      fn_entryProc = reinterpret_cast< VstEntryProc* >( GetProcAddress(dll, "main") );
      pin.m_entry="main";
   }
   if ( !fn_entryProc )
   {
      std::wcout << "No entry VSTPluginMain() or main() found, " << m_uri << std::endl;
      m_scanner->addBadPluginInfo( pin );
      return;
   }

   AEffect* m_vst = fn_entryProc( de::audio::vstHostCallbackDummy_static );
   if ( !m_vst )
   {
      std::wcout << "Not a VST plugin dll, " << m_uri << std::endl;
      m_scanner->addBadPluginInfo( pin );
      return;
   }
...
//   }
   ...
   de::audio::vstDispatch( m_vst, effOpen );
...
//   vstDispatch( m_vst, effEditClose );
   de::audio::vstDispatch( m_vst, effClose );               // Stop plugin
