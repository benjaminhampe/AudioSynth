...
//   setInputVolume( 100 ); // Boost means additional to original signal
   ...
   connect( m_qpeak, SIGNAL(valueChanged(int)), this, SLOT(on_qpeakChanged(int)), Qt::QueuedConnection );
}
...
void
GLowPass3::setBypassed( bool bypassed )
{
   m_menu->setBypassButton( bypassed );
}

void
GLowPass3::clearInputSignals()
{
   m_inputSignal = nullptr;
}

void
GLowPass3::setInputSignal( int i, de::audio::IDspChainElement* input )
{
   m_inputSignal = input;
}

void
GLowPass3::on_menuButton_bypass(bool checked)
{
   m_isBypassed = checked;
}

void
GLowPass3::on_menuButton_more(bool checked)
{
...
void
GLowPass3::on_menuButton_hideAll(bool checked)
{
   if ( checked )
   {
...
void
GLowPass3::on_freqChanged( int v )
{
   setBypassed( v < 1 );
   //computeCoefficients
}
...
void
GLowPass3::on_qpeakChanged( int v )
{
   //computeCoefficients();
}
...
uint64_t
GLowPass3::readSamples( double pts, float* dst, uint32_t dstFrames, uint32_t dstChannels, uint32_t dstRate )
{
