...
uint64_t
GSpektrometer::readSamples( double pts, float* dst, uint32_t dstFrames, uint32_t dstChannels, uint32_t dstRate )
{
...
// ============================================================================
{
   ...
   drawLine( m_img,a,b,de::RGBA(255,155,155) );

   a = computePos( 220.0f, db_min );
   b = computePos( 220.0f, db_max );
   drawLine( m_img,a,b,de::RGBA(255,200,155) );
   ...
   for ( size_t c = 0; c < m_channelCount; c++ )
   {
      ...
      uint32_t color = de::RGBA(255,255,32,100);
      if ( c == 0 )
      {
