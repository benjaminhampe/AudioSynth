...
//   }
      ...
      uint32_t color )
   {
      ...
      int w = pos.getWidth();
      int h = pos.getHeight();
      drawLine( img, 0,h/2,w-1,h/2, de::RGBA(255,255,255,255) );
...
//         int y2 = int( float(h/2) + float(h/2) * db / 60.0f );
         ...
         drawLine( img, k,y1,k+1,y2, de::RGBA(255,255,255,255) );
      }
   }
                 ...
                 de::Recti pos, uint32_t color )
   {
      ...
      int w = pos.getWidth();
      int h = pos.getHeight() / channelCount;
...
void GTimeSeek::timerEvent( QTimerEvent* event )
{
   if ( m_updateTimerID == event->timerId() )
   {
      update();
