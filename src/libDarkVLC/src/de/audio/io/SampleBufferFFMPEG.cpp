...
bool
SampleBufferFFMPEG::load( SampleBuffer & dst, std::string uri,
                       int audioStreamIndex, ESampleType sampleType, int sampleRate )
{
   ...
   {
      audioStreamIndex = file.getBestAudioStreamIndex();
      //DE_DEBUG("LOAD stream index = ", audioStreamIndex, " of ", file.getStreamCount() )
   }
   ...
   file.enableStream( audioStreamIndex ); // enable processing
   auto audioStream = file.getStream( audioStreamIndex );
   if ( !audioStream )
   {
      DE_ERROR("No audio stream")
      return false;
   ...
   SampleBufferList bufferList; // list of chunks -> join()ed at the end.
   ...
   auto appendBuffer =
      ...
      {
         //DE_INFO("Chunk[",chunks,"].payload->audioStreamIndex = ",payload->audioStreamIndex)
         //DE_INFO("Chunk[",chunks,"].payload->typ = ", ffUtils::toString( payload->typ ) )
         size_t localBytes = 0;
         double localDuration = 0.0;

         if ( payload->streamIndex == audioStreamIndex &&
...
bool
SampleBufferFFMPEG::save( SampleBuffer const & src, std::string uri )
{
   DE_DEBUG("uri ",uri)
   DE_FLUSH
