...
struct SampleBufferFFMPEG
{
   DE_CREATE_LOGGER("de.audio.SampleBufferFFMPEG")

   static bool
   load( SampleBuffer & dst,
         std::string uri,
   ...
   static bool
   save( SampleBuffer const & dst, std::string uri );
};
...
// The SampleBufferLoader always creates a ffmpeg instance that is independent -> thread safe.
