#ifndef DE_AUDIO_SAMPLETYPE_HPP
#define DE_AUDIO_SAMPLETYPE_HPP
...
//0 - A’  29,14   58,27   116,54  233,08  466,16  923,33  1864,66 3729,31
//0 - H   30,87   61,74   123,47  246,94  493,88  987,77  1975,53 3951,07

inline float
getFrequencyFromNote( int oktave, int semitone_C_is_zero, int detuneInCents = 0 )
{
   float fine = float( detuneInCents ) / 100.0f;
   return 440.f * powf( 2.f, float(oktave - 4) + ((float(semitone_C_is_zero - 9)+fine) / 12.f) );
}

inline float
getFrequencyFromMidiNote( int midiNote, int detuneInCents = 0 )
{
   int oktave = midiNote / 12;
   int semi = midiNote - 12 * oktave;
   //DEM_DEBUG("midiNote(",midiNote,") = oktave(",oktave,"),semi(",semi,")")
   return getFrequencyFromNote( oktave, semi, detuneInCents );
}
