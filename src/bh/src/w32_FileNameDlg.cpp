...
std::wstring loadFileNameDlgW()
{
   ...
   std::wstring uri = dbOpenFileDlgW(
      L"Load/open any image file (bmp,xpm,png,tga,jpg,tif,gif,dds,etc..)",
      dx,dy,dw,dh
   );
//            "Bitmap (*.bmp)\0*.bmp\0"
...
std::wstring saveFileNameDlgW()
{
   int dw = dbDesktopWidth() - 400;
   int dh = dbDesktopHeight() - 400;
   int dx = 200;
   int dy = 200;
   std::wstring uri = dbSaveFileDlgW(
