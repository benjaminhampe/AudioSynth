...
//      int oktave = midiNote / 12;
//      int semitone = midiNote - 12 * oktave;
//      std::cout << "Key[" << i << "] midiNote = " << midiNote << ", oktave = " << oktave << ", semitone = " << semitone << ", isBlackKey = " << de::midi::MidiTools::isBlackPianoKey(semitone) << std::endl;
//   }
...
void
KeyboardArea::enterEvent( QEvent* event )
{
   //m_isHovered = true;
   update();
