...
//   m_time_signature_z = 0;
//   m_time_signature_n = 1;
...
//   m_numNoteOff = 0;
//   m_numSetTempo = 0;
//   m_numProgramChanges = 0;
//   m_numControlChanges = 0;
//   m_numSustainCount = 0;
//   m_numSoftCount = 0;
}
...
MidiParser::parseFileHeader( uint8_t const* beg, uint8_t const* end )
{
   ...
   std::string s = dbStrJoin( "MIDI header signature, 8 bytes, has size 6." );
   emitToken( beg, src, s );
   ...
   // Debug Text
   s = dbStrJoin( "MIDI header filetype(", m_fmt, "), tracks(", m_trackCount,"), " );
   if ( m_resolution & 0x8000 ) // is bit 15 set in time resolution?
   {
      s += dbStrJoin("clock(SMPTE,", m_resolution,",[ticks per frame])" );
   }
   else
   {
      s += dbStrJoin("clock(normal,",m_resolution,"[ticks per quarter note])" );
   }

   // Debug Error Text
   if ( m_fmt == 0 && m_trackCount != 1 )
   {
      s += dbStrJoin(", WARN malformed format" );
      m_trackCount = 1;
   }
   ...
   {
      s += dbStrJoin(", WARN no m_ticksPerQuarterNote, fallback to 96." );
      m_ticksPerQuarterNote = 96;
   }
...
MidiParser::parseTrackHeader( uint8_t const* beg, uint8_t const* end, uint32_t & dataSize )
{
   ...
   auto s = dbStrJoin("NewTrack(",m_trackIndex,"/",m_trackCount,") header, dataSize = ", dataSize );
...
MidiParser::parseEvent( uint8_t const* beg, uint8_t const* end )
{
   ...
   if ( c1 < 0x80 )
   {
      if ( m_runStatus < 8 )
      {
         e = dbStrJoin( "- BAD runningStatus ", hexStr(m_runStatus) );
      }
      else
      {
         e = dbStrJoin( "- OK runningStatus ", hexStr(m_runStatus) );
      }
   ...
   if ( command == 8 || command == 9 )
   {
      ...
      if ( isOn )
      {
         auto s = dbStrJoin("dt(",dt,"), ch(",ch,") NOTE_ON(",MidiTools::noteStr(note),
      ...
      {
         auto s = dbStrJoin("dt(",dt,"), ch(",ch,") NOTE_OFF(",MidiTools::noteStr(note),
   ...
   else if ( command == 10 )
   {
      ...
      s = dbStrJoin("dt(",dt,"), ch(",ch,") AfterTouch(",value,"), "
   ...
   else if ( command == 11 )
   {
      ...
      s = dbStrJoin("dt(",dt,"), ch(",ch,") ",ccStr,"(0x",hexStr(d2),"), runstat(0x", hexStr(m_runStatus),")" );
      emitToken( beg, src, s );
   ...
   {
      s = dbStrJoin("dt(",dt,"), ch(",ch,") ",EMidiGM_toString( EMidiGM(d1) ), "(0x",hexStr(d1),"), runstat(0x", hexStr(m_runStatus),")" );
      emitToken( beg, src, s );
   ...
   {
      s = dbStrJoin("dt(",dt,"), ch(",ch,") ChannelPressure, d1(0x",hexStr(d1),"), runstat(0x", hexStr(m_runStatus),")" );
      emitToken( beg, src, s );
   ...
   else if ( command == 14 )
   {
      ...
      s = dbStrJoin("dt(",dt,"), ch(",ch,") PitchBend(",bend,"), d1(0x",hexStr(d1),"), d2(0x",hexStr(d2),"), runstat(0x", hexStr(m_runStatus),")" );
      emitToken( beg, src, s );
         ...
         case 0: // F0 - Begin System Exclusive [SysEx]
          {
            ...
            s = dbStrJoin("dt(",dt,") F0 - Sysex begin, runstat(0x", hexStr(m_runStatus),")" );
            emitToken( beg, src, s );
          }
            break;
         case 1: // F1 - Time Code QuarterNote Frame
            d2 = *src++;
            s = dbStrJoin("dt(",dt,") F1 - TimeCode Quarter Frame(",int(d2),"), runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
         case 2: // F2 - Song Position Pointer
            d2 = *src++;
            s = dbStrJoin("dt(",dt,") F2 - Song Position Pointer(",int(d2),"), runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
         case 3: // F3 - Song Select (Song #) 1byte (0-127)
            d2 = *src++;
            s = dbStrJoin("dt(",dt,") F3 - Song Select(",int(d2),"), runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
         case 4: // F4 - Undefined (Reserved)
            s = dbStrJoin("dt(",dt,") F4 - Undefined (Reserved), runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
         case 5: // F5 - Undefined (Reserved)
            s = dbStrJoin("dt(",dt,") F5 - Undefined (Reserved), runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
         case 6: // F6 - TuneRequest
            s = dbStrJoin("dt(",dt,") NOIMPL F6 - TuneRequest, runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
         case 7: // F7 - End System Exclusive
          {
            ...
            s = dbStrJoin("dt(",dt,") F7 - SysEx end., runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
          }
            break;
         case 8: // F8 - Timing clock
            s = dbStrJoin("dt(",dt,") NOIMPL F8 - Timing clock, runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
         case 9: // F9 - Undefined (Reserved)
            s = dbStrJoin("dt(",dt,") NOIMPL F9 - Undefined (Reserved), runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
         case 10: // FA - MidiClock Start
            s = dbStrJoin("dt(",dt,") NOIMPL FA - MidiClock Start, runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
         case 11: // FB - MidiClock Continue
            s = dbStrJoin("dt(",dt,") NOIMPL FB - MidiClock Continue, runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
         case 12: // FC - MidiClock Stop
            s = dbStrJoin("dt(",dt,") NOIMPL FC - MidiClock Stop, runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
         case 13: // FD - Undefined (Reserved)
            s = dbStrJoin( "dt(",dt,") NOIMPL FD - Undefined (Reserved), runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
         case 14: // FE - Active Sensing
            s = dbStrJoin( "dt(",dt,") NOIMPL FE - Active Sensing, runstat(0x", hexStr(m_runStatus),")");
            emitToken( beg, src, s );
            break;
                           ...
                           uint8_t metatype )
{
   ...
   if ( metatype == 0 )
   {
      ...
      uint8_t c4 = *src++;
      int number = c4;
      s = dbStrJoin( "dt(",dt,") FF 00 02 - SequenceNumber(",number,")" );
      if ( c3 != 0x02 )
      {
   ...
   else if ( metatype > 0 && metatype < 10 )
   {
      ...
      else
      {
         ...
         if      (metatype == 1) // text
         {
            s = dbStrJoin("dt(",dt,") FF 01 - Text(",dat,"), dataSize = ", dataSize );
         }
         else if (metatype == 2) // copyright
         {
            s = dbStrJoin("dt(",dt,") FF 02 - Copyright(",dat,"), dataSize = ", dataSize );
         }
         else if (metatype == 3) // track name
         {
            s = dbStrJoin("dt(",dt,") FF 03 - TrackName(",dat,"), dataSize = ", dataSize );
            m_curTrack.m_name = dat;
         }
         else if (metatype == 4) // instrument name
         {
            s = dbStrJoin("dt(",dt,") FF 04 - InstrumentName(",dat,"), dataSize = ", dataSize );
         }
         else if (metatype == 5) // lyric
         {
            s = dbStrJoin("dt(",dt,") FF 05 - LyricText(",dat,"), dataSize = ", dataSize );
         }
         else if (metatype == 6) // marker
         {
            s = dbStrJoin("dt(",dt,") FF 06 - MarkerText(",dat,"), dataSize = ", dataSize );
         }
         else if (metatype == 7) // cue point
         {
            s = dbStrJoin("dt(",dt,") FF 07 - CuePoint(",dat,"), dataSize = ", dataSize );
         }
         else if (metatype == 8) // program name
         {
            s = dbStrJoin("dt(",dt,") FF 08 - ProgramName(",dat,"), dataSize = ", dataSize );
         }
         else if (metatype == 9) // device name
         {
            s = dbStrJoin("dt(",dt,") FF 09 - DeviceDame(",dat,"), dataSize = ", dataSize );
         }
   ...
   else if (metatype == 0x20)
   {
      ...
      uint8_t c4 = *src++;
      s = dbStrJoin("dt(",dt,") FF 20 01 - Channel prefix ", int(c4) );
      if ( c3 != 0x01 )
      {
   ...
   else if (metatype == 0x21)
   {
      uint8_t c3 = *src++;
      int port = *src++;
      s = dbStrJoin("dt(",dt,") FF 21 (01)? - MIDI port display = ", port );
      if (c3 != 0x01)
      {
   ...
   {
      uint8_t c3 = *src++;
      s = dbStrJoin("dt(",dt,") FF 2F 00 - EndOfTrack" );
      if (c3 != 0x00)
      {
   ...
   else if (metatype == 0x51)
   {
      ...
      s = dbStrJoin("dt(",dt,") FF 51 03 - SetTempo(", tempo, "), microseconds(",us, ")" );
      if ( c3 != 0x03 )
      {
   ...
   else if (metatype == 0x54)
   {
      ...
      s = dbStrJoin("dt(",dt,") FF 54 05 - SMPTE(",hh,":",mm,":",ss,"), fc(",fc,"), subfc(",sf,")");
      if ( c3 != 0x05 )
      {
   ...
   else if (metatype == 0x58)
   {
      ...
      s = dbStrJoin("dt(",dt,") FF 58 04 - TimeSignature(",nn,"/",dd,"), cpb(",cc,"), 32nd_notes_per_beat(",bb,")" );
      if ( c3 != 0x04 )
      {
   ...
   else if (metatype == 0x59) // key signature
   {
      ...
      s = dbStrJoin("dt(",dt,") FF 59 02 - KeySignature(Cdur + ",tonart,")"
          ", minor(",minor,")" );
      if ( c3 != 0x02 )
      {
         s += " ERROR c3(0x" + hexStr(c3) + ") != 0x02";
   ...
   {
      uint8_t c3 = *src++;
      s = dbStrJoin("dt(",dt,") FF 7F ", hexStr(c3), " - UNKNOWN, try skip" );
      if ( c3 < 1 )
      {
   ...
   else
   {
      ...
      s = dbStrJoin("dt(",dt,") FF ",hexStr(metatype)," - UNKNOWN META TYPE"
                                                      " :: Probably FATAL." );
