...
uint64_t
DecoderTools::extractAudio( AVFrame* avFrame, Payload & payload, bool debug )
{
...
int32_t
DecoderTools::readFrame( DecoderCtx & ds, ON_VIDEO_PAYLOAD const & onPayload )
{
...
DecoderTools::openDecoder( DecoderCtx & ds, std::string uri, int streamIndex, bool debug )
{
   ...
   if ( streamIndex < 0 )
   {
      streamIndex = av_find_best_stream( ds.formatCtx, AVMEDIA_TYPE_AUDIO, -1, -1, nullptr, 0 );
   }
