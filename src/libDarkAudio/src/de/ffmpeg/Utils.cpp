...
de::ColorFormat::EColorFormat
Utils::toEColorFormat( AVPixelFormat pixelFormat )
{
   switch( pixelFormat )
   {
   case AV_PIX_FMT_RGB24: return de::ColorFormat::RGB888;
   case AV_PIX_FMT_RGBA:  return de::ColorFormat::RGBA8888;
   case AV_PIX_FMT_BGR0:  return de::ColorFormat::BGRA8888;
   case AV_PIX_FMT_BGR24:  return de::ColorFormat::BGR888;
   default: DE_ERROR( "Invalid pixel format" ) return de::ColorFormat::Unknown;
   }
}
...
Utils::dumpStreamInfos( AVFormatContext* file, bool dumpInfoFilePerStream )
{
   ...
   std::string fileName = dbGetFileName( uri );
   ...
   for ( uint32_t i = 0; i < streamCount; ++i )
   {
      ...
      {
         DE_DEBUG(name,"Stream[",i,"].Duration = ", dbSecondsToString( t * stream->duration ) )
      }
      ...
      {
         DE_DEBUG(name,"Stream[",i,"].StartTime = ", dbSecondsToString( t * stream->start_time ) )
      }
   ...
   double pc )
{
   ...
   int64_t ts = int64_t( de::Math::clampd( pc, 0.0, 1.0 ) * stream->duration );
   ...
   int scale_quality )
{
   ...
   if ( img.getFormat() != de::ColorFormat::RGB888 &&
        img.getFormat() != de::ColorFormat::RGBA8888 )
   {
...
//   if ( m_avCodecCtx->codec_type != AVMEDIA_TYPE_VIDEO ) { DE_ERROR("Not an image stream" ) return false; }
   ...
   if ( img.getWidth() != dst_w || img.getHeight() != dst_h )
   {
   ...
   int dst_linesize[4] = { int( img.getStride() ), 0, 0, 0 };
   ...
   de::audio::ESampleType dst_convert )
{
   ...
   de::audio::ESampleType dstType = dst.getSampleType();
...
#if 0
      // I do this to overcome any padding (unpacked) memory between channels
      // Buffer is (tightly) packet and easier to manipulate.
//      for ( size_t c = 0; c < channelCount; ++c )
//      {
//         uint8_t const * pSrc = reinterpret_cast< uint8_t const* >(avFrame->extended_data[ c ]);
//         uint8_t* pDst = dst.getSamples( 0, c );
//         converter( pSrc, pDst, srcFrames );
//      }

            // I do this to overcome any padding (unpacked) memory between channels
      // Buffer is (tightly) packet and easier to manipulate.

      uint32_t src_bps = de::audio::ST_getBytesPerSample( srcType );
      int32_t lineSize = avFrame->linesize[0];
      int32_t osrcFrames = lineSize / src_bps;
      //dst.resize( osrcFrames );
      DE_DEBUG("PLANAR lineSize(",lineSize,"), src_bps(",src_bps,"), srcFrames(",srcFrames,"), osrcFrames(",osrcFrames,"), dst(", dst.toString(),")")

      for ( uint32_t c = 0; c < channelCount; ++c )
      {
         uint8_t const * pSrcStart = reinterpret_cast< uint8_t const* >(avFrame->extended_data[ c ] );
         for ( size_t i = 0; i < srcFrames; ++i )
         {
            uint8_t const * pSrc = pSrcStart + i * src_bps;
            uint8_t* pDst = dst.getSamples( i, c );
