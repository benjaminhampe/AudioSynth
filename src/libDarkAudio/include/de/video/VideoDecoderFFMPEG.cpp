...
float
VideoDecoderTools::getSample( AVSampleFormat const & fmt, uint8_t const* src, uint32_t srcBPS )
{
   int64_t val = 0;
   float ret = 0;
   switch( srcBPS )
...
bool
VideoDecoderTools::isInterleaved( AVSampleFormat fmt )
{
   switch ( fmt )
   {
...
uint64_t
VideoDecoderTools::extractAudio( AVFrame* avFrame, VideoPayload & payload, bool debug )
{
...
int32_t
VideoDecoderTools::readFrame( VideoDecoderCtx & ds, ON_VIDEO_PAYLOAD const & onPayload )
{
