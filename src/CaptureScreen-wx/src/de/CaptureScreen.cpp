...
std::vector< uint8_t >
toPixelVector( HBITMAP hBitmap, int & w, int & h )
{
   ...
   info.bmiHeader.biWidth = w = bmp.bmWidth;
   info.bmiHeader.biHeight = h = bmp.bmHeight;
   info.bmiHeader.biPlanes = 1;
   info.bmiHeader.biBitCount = bmp.bmBitsPixel;
   info.bmiHeader.biCompression = BI_RGB;
   info.bmiHeader.biSizeImage = ((w * bmp.bmBitsPixel + 31) / 32) * 4 * h;
   std::vector< uint8_t > pixels( info.bmiHeader.biSizeImage );
   GetDIBits( dc, hBitmap, 0, h, pixels.data(), &info, DIB_RGB_COLORS );
...
de::Image
toDarkImage( HBITMAP hBitmap )
{
   int w = 0;
   int h = 0;
   std::vector< uint8_t > pixels = toPixelVector( hBitmap, w, h );
   de::Image img( w,h );
   //img.fill( 0xFFFFFFFF );
...
CaptureScreen::createImageFromDesktop()
{
   ...
   de::Image img = toDarkImage( hBitmap );
