...
uint64_t
GCrossFader::readSamples( double pts, float* dst, uint32_t dstFrames, uint32_t dstChannels, uint32_t dstRate )
{
   ...
   if ( m_leftSignal )
   {
      m_leftSignal->readSamples( pts, m_leftBuffer.data(), dstFrames, dstChannels, dstRate );
   }

   if ( m_rightSignal )
   {
      m_rightSignal->readSamples( pts, m_rightBuffer.data(), dstFrames, dstChannels, dstRate );
   }
