   ...
   , m_directoryMultiByte{} // needed for audioMasterGetDirectory
{
   ...
   h->setSpacing( 1 );
   h->addWidget( m_menu );
   h->addWidget( m_editorImage );
   h->addWidget( m_loadButton );
   ...
   connect( m_loadButton, SIGNAL(clicked(bool)), this, SLOT(on_loadButton(bool)), Qt::QueuedConnection );
   connect( m_volumeSlider, SIGNAL(valueChanged(int)), this, SLOT(on_volumeSlider(int)), Qt::QueuedConnection );
...
void
GVstPlugin::on_menuButton_bypass(bool checked)
{
   m_isBypassed = checked;
}

void
GVstPlugin::on_menuButton_more(bool checked)
{
...
void
GVstPlugin::on_menuButton_hideAll(bool checked)
{
...
GVstPlugin::on_editorClosed()
{
   ...
   m_menu->m_btnMore->blockSignals( true );
   m_menu->m_btnMore->setChecked( false );
   m_menu->m_btnMore->blockSignals( false );
}

void
GVstPlugin::on_volumeSlider( int v )
{
   m_volume = v;
}

void
GVstPlugin::on_loadButton( bool checked )
{
...
void
GVstPlugin::setBypassed( bool enabled )
{
   m_isBypassed = enabled;
}

void
GVstPlugin::clearInputSignals()
{
   m_inputSignal = nullptr;
}

void
GVstPlugin::setInputSignal( int i, de::audio::IDspChainElement* input )
{
   m_inputSignal = input;
}

void
GVstPlugin::setupDspElement( uint32_t dstFrames, uint32_t dstChannels, uint32_t dstRate )
{
...
uint64_t
GVstPlugin::readSamples( double pts, float* dst, uint32_t dstFrames, uint32_t dstChannels, uint32_t dstRate )
{
