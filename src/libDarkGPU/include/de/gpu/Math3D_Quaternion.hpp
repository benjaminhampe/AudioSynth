#ifndef BENNI_GPU_QUATERNION_HPP
#define BENNI_GPU_QUATERNION_HPP
...
struct Direction3 // Another name for 'direction' that can be moved parallel without change.
{
   ...
   toEulerAngles( V3 dir )
   {
      ...
      T TWOPI = T( Math::TWOPI64 );
      if ( euler.y < T( 0 ) ) euler.y += TWOPI;
      if ( euler.y >= TWOPI ) euler.y -= TWOPI;
      euler.x = dbAtan2( dbSqrt( dir.x*dir.x + dir.z*dir.z ), dir.y ) - T( Math::HALF_PI64 );
      if ( euler.x < T( 0 ) ) euler.x += TWOPI;
      if ( euler.x >= TWOPI ) euler.x -= TWOPI;
      return euler * T( Math::RAD2DEG );
   }
...
struct YawPitchRoll
{
   DE_CREATE_LOGGER("de.YawPitchRoll")
   typedef glm::tvec3< T > V3;
   ...
   toString() const
   {
      ...
      << "yaw = " << Math::toDeg( yaw ) << ","
      << "pitch = " << Math::toDeg( pitch ) << ","
      << "roll = " << Math::toDeg( roll );
      return s.str();
   }
   ...
   {
      return YawPitchRoll( Math::toRad( y ), Math::toRad( p ), Math::toRad( r ) );
   }
   ...
   T getYawDegrees() const { return Math::toDeg( yaw ); }

   T getPitch() const { return pitch; }
   T getPitchDegrees() const { return Math::toDeg( pitch ); }

   T getRoll() const { return roll; }
   T getRollDegrees() const { return Math::toDeg( roll ); }

   T getMinYaw() const { return Math::toRad( T(0.0) ); }
   T getMaxYaw() const { return Math::toRad( T(360.0) ); }

   T getMinPitch() const { return Math::toRad( T(-89.0) ); }
   T getMaxPitch() const { return Math::toRad( T(89.0) ); }

   T getMinRoll() const { return Math::toRad( T(-89.0) ); }
   T getMaxRoll() const { return Math::toRad( T(89.0) ); }
...
struct Quat
{
   DE_CREATE_LOGGER("de.math.Quaternion")
...
template < typename T >
std::ostream& operator<< ( std::ostream& o, de::gpu::EulerAngles< T > const & euler )
{
   o << euler.toString();
   return o;
}
...
template < typename T >
std::ostream& operator<< ( std::ostream& o, de::gpu::AngleAxis< T > const & euler )
{
   o << euler.toString();
   return o;
}
...
template < typename T >
std::ostream& operator<< ( std::ostream& o, de::gpu::Quat< T > const & quat )
{
   o << quat.toString();
   return o;
}
