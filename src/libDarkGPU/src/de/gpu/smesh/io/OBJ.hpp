...
#ifndef DE_GPU_SMESH_LOAD_OBJ_MTL_HPP
#define DE_GPU_SMESH_LOAD_OBJ_MTL_HPP

#include <de/gpu/smesh/SMesh.hpp>
#include <de/gpu/smesh/io/MTL.hpp>
...
struct SMeshOBJ
{
   ...
   static bool
   save( SMeshBuffer const & buf, std::string const & objUri, IVideoDriver* driver )
   {
      std::string mtlUri = dbGetFileBaseName( objUri ) + ".mtl";
      DE_DEBUG("Write SMeshBuffer to MTL = ", mtlUri )
      DE_DEBUG("Write SMeshBuffer to OBJ = ", objUri )

      std::vector< SMaterial > matList;
      matList.reserve( 1 );
      matList.emplace_back();
      SMaterial & mat = matList.back();
      mat = buf.Material;
      ...
      s << "# " << PrimitiveType::getString( buf.PrimType ) << "\n";
      s << "o " << bufName << "\n";
...
//         }
   ...
   static bool
   save( SMesh const & mesh, std::string const & objUri, IVideoDriver* driver )
   {
      std::string mtlUri = dbGetFileBaseName( objUri ) + ".mtl";
      DE_DEBUG("Writing MTL = ", mtlUri )
      DE_DEBUG("Writing OBJ = ", objUri )
      ...
      for ( size_t i = 0; i < mesh.getMeshBufferCount(); ++i )
      {
         matList.emplace_back();
         SMaterial & mat = matList.back();
         mat = mesh.getMeshBuffer( i ).Material;
      ...
      for ( size_t b = 0; b < mesh.getMeshBufferCount(); ++b )
      {
         ...
         std::string matName = src.Material.Name;
         if ( matName.empty() )
         {
            matName = "SMaterial";
         ...
         s << "# " << PrimitiveType::getString( src.PrimType ) << "\n";
         s << "o " << bufName << "\n";
