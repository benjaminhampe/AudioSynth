...
struct SMeshLoad3DSFile
{
   ...
   bool
   load( SMesh & out, Binary & file, IVideoDriver* driver )
   {
      Mesh = &out;
      m_Driver = driver;
      ...
      if ( ok )
      {
         DE_DEBUG("Mesh ", Mesh->toString() )
         DE_DEBUG("Mesh recalculate bounding box" )
         Mesh->recalculateBoundingBox( true );
         DE_DEBUG("Mesh.BufferCount = ", Mesh->getMeshBufferCount() )

         for ( uint32_t i=0; i<Mesh->getMeshBufferCount(); ++i )
         {
            DE_DEBUG("Mesh.Buffer[",i,"].Material = ", Mesh->getMeshBuffer(i).getMaterial().toString() )
         }
      }
...
void SMeshLoad3DSFile::loadMaterials( de::Binary & file )
{
   ...
   for (uint32_t i=0; i<Materials.size(); ++i)
   {
      ...
      if ( matData.Filename[0].size() )
      {
         ...
         if ( uri.size() )
         {
            TexRef ref = m_Driver->addTexture2D( uri );
            if (!ref.tex)
            {
      ...
      if (matData.Filename[1].size())
      {
         ...
         if ( uri.size() )
         {
            ...
            {
               m->getMaterial().setTexture(1, ref.tex );
            }
         }
      }
...
bool
SMeshLoad3DS::load( SMesh & mesh, de::Binary & file, IVideoDriver* driver )
{
