...
//static
bool
AssimpTools::parseMaterial( aiScene const * const assimpScene, uint32_t matIndex, SMaterial & out, IVideoDriver* driver, std::string baseDir )
{
   // Validate param 1
   ...
   {
      out.state.culling = Culling::enabled( ivalue > 0 );
   }
...
//               }
   ...
   if ( textures.size() > 0 && driver )
   {
      ...
      {
         SO so; // TODO: read some data into here
         std::string uri = textures[ k ].m_FileName;
         TexRef ref = driver->getTexture( uri );
         if ( !ref.tex )
         {
            ref = driver->addTexture2D( uri, uri, false, so );
         }
