                           ...
                           glm::vec2 radius, uint32_t color, TexRef ref )
{
   ...
   bb.mesh.Material.state.blend = Blend::alphaBlend();
...
void
BillboardRenderer::render( IVideoDriver* driver )
{
   ...
   {
      draw3D( bb.mesh, driver );
   }
}

void
BillboardRenderer::draw3D( IMeshBuffer & vao, IVideoDriver* driver )
{
   if ( !driver ) { DE_ERROR("No driver") return; }
   ...
   IShader* shader = driver->getShader( name );
   if ( !shader )
   {
      ...
      shader = driver->addShader( name, vs.str(), fs.str() );
   }
   ...
   // [UseProgram]
   if ( !driver->setActiveShader( shader ) )
   {
   ...
   auto camera = driver->getActiveCamera();
   if ( camera )
   {
   ...
   if ( texDiffuse.tex )
   {
      int stage = driver->bindTexture( texDiffuse.tex );
      if ( stage < 0 )
      {
   ...
   driver->setState( vao.getMaterial().state );

   driver->drawPrimitiveList( vao );
