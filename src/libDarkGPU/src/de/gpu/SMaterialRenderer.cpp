...
SMaterialRenderer::SMaterialRenderer( IVideoDriver* driver )
   : m_Driver( driver )
   , m_ScreenGamma( 1.25f )
{
   addLight( 1,
...
SMaterialRenderer::setMaterial( SMaterial const & material )
{
   ...
   // [UseProgram]
   IShader* shader = getShader( material );
   if ( !shader )
   {
...
//      }
   ...
   if ( texDiffuse.tex )
   {
      int stage = m_Driver->bindTexture( texDiffuse.tex );
      if ( stage < 0 )
      {
...
//      }
      ...
      if ( !shader->setUniform2f( "u_texDiffuseRepeat", texDiffuse.repeat ) )
      {
   ...
   if ( texBump.tex )
   {
      int stage = m_Driver->bindTexture( texBump.tex );
      if ( stage < 0 )
      {
...
//      }
      ...
      if ( !shader->setUniform2f( "u_texBumpRepeat", texBump.repeat ) )
      {
...
SMaterialRenderer::getShader( SMaterial const & material )
{
   ...
   else
   {
      ...
      if ( !shader )
      {
         shader = m_Driver->addShader( name, createVS( material ), createFS( material ) );
      }
...
//   }
   ...
   if ( material.hasDiffuseMap() &&
        ( material.CloudTransparent ||
          material.getDiffuseMap().tex->hasTransparency() ) )
   {
      fs << R"(
