...
// ===========================================================================
void destroyRT( RT & rt, IVideoDriver* driver )
// ===========================================================================
{
   GLint fbo = 0;
   ...
   if ( driver )
   {
      driver->unbindTexture2D( rt.colorTex );
      driver->unbindTexture2D( rt.depthTex );
   }
   // Destroy fbo.
   if ( rt.fbo )
   {
...
// ===========================================================================
bool createRT( RT & rt, int w, int h, IVideoDriver* driver )
// ===========================================================================
{
   if ( !driver )
   {
      DEM_ERROR("No driver, cant bind or unbind textures")
      return false;
   }
   ...
   if ( !driver->bindTexture2D( colorTex ) )
   {
      DEM_ERROR("Cant bind colorTex")
   }
   ...
   if ( !driver->bindTexture2D( depthStencilTex ) )
   {
      DEM_ERROR("Cant bind depthStencilTex")
   }
   ...
   if (!ok)
   {
      ...
      DEM_ERROR("Incomplete FBO ", msg )
   }
   else
   {
      DEM_DEBUG("Created RT FBO w:",w,", h:",h)
   }

   // Restore old FBO
   ::glBindFramebuffer( GL_FRAMEBUFFER, old_fbo ); GL_VALIDATE

   if ( !driver->unbindTexture2D( colorTex ) )
   {
      DEM_ERROR("Cant unbind colorTex")
   }
   if ( !driver->unbindTexture2D( depthStencilTex ) )
   {
      DEM_ERROR("Cant unbind depthStencilTex")
   }
