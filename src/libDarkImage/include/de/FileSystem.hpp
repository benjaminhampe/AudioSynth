#ifndef DARKFILESYSTEM_HPP
#define DARKFILESYSTEM_HPP
...
struct FileSystem
{
   ...
   static bool
   copyFile( std::string const & src, std::string const & dst );
...
inline std::string dbLoadText( std::string const & uri ) { return de::FileSystem::loadText( uri ); }
inline bool dbSaveText( std::string const & uri, std::string const & txt ) { return de::FileSystem::saveText( uri, txt ); }
inline std::vector< std::string > dbLoadTextLn( std::string const & uri ) { return de::FileSystem::loadTextLines( uri ); }
inline std::vector< std::string > dbLoadTextLnFromMemory( char const * xpm_data ) { return de::FileSystem::loadTextLnFromMemory( xpm_data ); }
inline bool dbExistFile( std::string const & uri ) { return de::FileSystem::existFile( uri ); }
inline bool dbExistFile( std::wstring const & uri ) { return de::FileSystem::existFile( uri ); }
inline void dbRemoveFile( std::string const & uri ) { de::FileSystem::removeFile( uri ); }
inline void dbRemoveFile( std::wstring const & uri ) { de::FileSystem::removeFile( uri ); }

inline bool dbExistDirectory( std::string const & uri ) { return de::FileSystem::existDirectory( uri ); }
inline void dbMakeDirectory( std::string const & uri ) { de::FileSystem::createDirectory( uri ); }
inline void dbRemoveDirectory( std::string const & uri ) { de::FileSystem::removeDirectory( uri ); }
inline std::string dbGetParentDir( std::string uri ) { return de::FileSystem::parentDir( uri ); }
inline std::string dbGetFileDir( std::string const & uri ) { return de::FileSystem::parentDir( uri ); }
inline std::string dbGetFileSuffix( std::string const & uri ) { return de::FileSystem::fileSuffix( uri ); }
inline bool dbIsAbsolute( std::string const & uri ) { return de::FileSystem::isAbsolute( uri ); }
inline std::string dbGetFileBaseName( std::string const & uri ) { return de::FileSystem::fileBase( uri ); }
inline int64_t dbGetFileSize( std::string const & uri ) { return de::FileSystem::fileSize( uri ); }
inline std::string dbGetFileName( std::string const & uri ) { return de::FileSystem::fileName( uri ); }
inline std::string dbCWD() { return de::FileSystem::cwd(); }
inline bool dbCopyFile( std::string const & src, std::string const & dst ) { return de::FileSystem::copyFile( src, dst ); }
inline bool dbCopyDirectory( std::string const & src, std::string const & dst ) { return de::FileSystem::copyDirectory( src, dst ); }
inline void dbGetEntries(  std::string baseDir, bool recursive, bool withFiles, bool withDirs,
         std::function< void( std::string const & ) > const & userFunc ) { de::FileSystem::entryList( baseDir, recursive, withFiles, withDirs, userFunc ); }
