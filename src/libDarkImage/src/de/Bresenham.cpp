...
Bresenham::traverseEllipse( int32_t a, int32_t b, SetPixel_t const & setPixel )
{
   ...
   int32_t F = Math::round32( float32_t( bb - aa * b ) + float32_t( aa ) * 0.25f );
   while ( ddF_x < ddF_y )
   {
      if ( F >= 0 )
      {
   ...
   // Bresenham Alghorithmus
   F = Math::round32( float32_t( bb ) * ( float32_t( x ) + 0.5f ) * ( float32_t( x ) + 0.5f ) +
                      float32_t( aa*(y-1)*(y-1) - aa*bb ) );
   while ( y > 0 )
   {
      if ( F <= 0 )
      {
...
//   }
   ...
   // Bresenham Alghorithmus
   int32_t F = Math::round32( float32_t( b2-a2*b ) + float32_t( a2 )*0.25f );
   while ( ddF_x < ddF_y )
   {
      if (F >= 0)
      {
   ...
   // Bresenham Alghorithmus
   F = Math::round32( float32_t( b2 ) * ( float32_t( x ) + 0.5f ) * ( float32_t( x ) + 0.5f )
                     + float32_t( a2*(y-1)*(y-1) - a2*b2 ) );
   while ( y > 0 )
   {
      if ( F <= 0 )
      {
   ...
   bool blend )
{
   ...
   float32_t const A1=(f32)color_a.getAlpha();
   float32_t const R1=(f32)color_a.getRed();
   float32_t const G1=(f32)color_a.getGreen();
   float32_t const B1=(f32)color_a.getBlue();
   float32_t const dA=(f32)color_b.getAlpha()-A1;
   float32_t const dR=(f32)color_b.getRed()-R1;
   float32_t const dG=(f32)color_b.getGreen()-G1;
   float32_t const dB=(f32)color_b.getBlue()-B1;
   ...
   bool blend )
{
   if (border==0)
   {
      ...
      float32_t const n_inv = irr::core::reciprocal( (f32)(x2 - x1) * (f32)(y2 - y1) );

      for (int32_t y=y1; y<=y2; y++)
      {
         for (int32_t x=x1; x<=x2; x++)
         {
            float32_t const f11 = (f32)(x2 - x) * (f32)(y2 - y) * n_inv; // bottomRight
            float32_t const f21 = (f32)(x - x1) * (f32)(y2 - y) * n_inv; // bottomLeft
            float32_t const f12 = (f32)(x2 - x) * (f32)(y - y1) * n_inv; // topRight
            float32_t const f22 = (f32)(x - x1) * (f32)(y - y1) * n_inv; // topLeft

            float32_t const fA = 	(f32)topLeft.getAlpha() * f11 +
                        (f32)topRight.getAlpha() * f21 +
                        (f32)bottomRight.getAlpha() * f22 +
                        (f32)bottomLeft.getAlpha() * f12;

            float32_t const fR = 	(f32)topLeft.getRed() * f11 +
                        (f32)topRight.getRed() * f21 +
                        (f32)bottomRight.getRed() * f22 +
                        (f32)bottomLeft.getRed() * f12;

            float32_t const fG = 	(f32)topLeft.getGreen() * f11 +
                        (f32)topRight.getGreen() * f21 +
                        (f32)bottomRight.getGreen() * f22 +
                        (f32)bottomLeft.getGreen() * f12;

            float32_t const fB = 	(f32)topLeft.getBlue() * f11 +
...
//
   ...
   else if ( border > 1)
   {
      ...
      float32_t const n_inv = irr::core::reciprocal( (f32)w * (f32)h );
      ...
      for (u32 y = 0; y < h; y++)
      {
         ...
         while (x < w)
         {
            ...
            if (colorNow == white)
            {
               float32_t const f11 = (f32)(w-1-x) * (f32)(h-1-y) * n_inv; // topLeft
               float32_t const f21 = (f32)(x) * (f32)(h-1-y) * n_inv; // topRight
               float32_t const f12 = (f32)(w-1-x) * (f32)(y) * n_inv; // bottomLeft
               float32_t const f22 = (f32)(x) * (f32)(y) * n_inv; // bottomRight
...
void Tool::drawRoundRect( IImage* dst, const core::recti& pos, int32_t rx, int32_t ry, const SColor& topLeft, const SColor& topRight, const SColor& bottomRight, const SColor& bottomLeft, int32_t border, bool blend)
{
   ...
   for (u32 y=0; y<size.Height; y++)
   {
      for (u32 x=0; x<size.Width; x++)
      {
         ...
         {
            float32_t const f11 = (f32)(size.Width-1-x) * (f32)(size.Height-1 - y) * n_inv; // topLeft
            float32_t const f21 = (f32)(x) * (f32)(size.Height-1 - y) * n_inv; // topRight
            float32_t const f12 = (f32)(size.Width-1 - x) * (f32)(y) * n_inv; // bottomLeft
            float32_t const f22 = (f32)(x) * (f32)(y) * n_inv; // bottomRight
...
//
//			float32_t const fA = (f32)c1.getAlpha(), fdA = (f32)c2.getAlpha() - fA;
//			float32_t const fR = (f32)c1.getRed(), fdR = (f32)c2.getRed() - fR;
//			float32_t const fG = (f32)c1.getGreen(), fdG = (f32)c2.getGreen() - fG;
//			float32_t const fB = (f32)c1.getBlue(), fdB = (f32)c2.getBlue() - fB;
//			const SColor white(255,255,255,255);
//			float32_t const fw = 0.5f*(f32)tmp->getDimension().Width;
//			float32_t const fh = 0.5f*(f32)tmp->getDimension().Height;
//			float32_t const invRadius = irr::core::reciprocal( (f32)radius );
//
...
//					if (colorNow == white)
//					{
//						float32_t const dx = (f32)x - fw;
//						float32_t const dy = (f32)y - fh;
...
//				const core::dimension2du& Size = tmp->getDimension();
//
//				float32_t const fw = 0.5f*(f32)Size.Width;
//				float32_t const fh = 0.5f*(f32)Size.Height;
//				float32_t const invRadius = irr::core::reciprocal( (f32)border );
