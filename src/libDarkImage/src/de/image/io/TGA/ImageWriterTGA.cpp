...
ImageWriterTGA::save( Image const & img, std::string const & uri, uint32_t quality )
{
   ...
   header.YOrigin[0] = 0;
   header.YOrigin[1] = 0;
   header.Width = uint16_t( img.getWidth() );
   header.Height = uint16_t( img.getHeight() );
   ...
   size_t row_size = header.getBytesPerPixel() * header.Width;
   ...
   for ( uint32_t y = 0; y < header.Height; ++y )
   {
      ...
      if ( img.getFormat() == ColorFormat::RGB888 )
      {
         ColorConverter::convert24BitTo24Bit( &src[ y * pitchBytes], row_pointer, header.Width, 1, 0, 0, true );
      }
      else
      {
         converter( &src[ y * pitchBytes ], row_pointer, header.Width );
      }
