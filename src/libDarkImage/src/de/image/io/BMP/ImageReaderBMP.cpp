...
//   }
   ...
   if ( bpp == 24 )
   {
      converter = ColorConverter::getConverter( ColorFormat::BGR888, ColorFormat::RGB888 );
      ...
      file.read( reinterpret_cast< char* >( data.data() ), dataBytes );
      file.close();

      void const * pSrc = data.data();
      void * pDst = img.data();
      converter( pSrc, pDst, size_t( w ) * h );
   ...
   {
      converter = ColorConverter::getConverter( ColorFormat::BGRA8888, ColorFormat::RGBA8888 );
   ...
   else if ( bpp == 8 && header.Compression == 0 )
   {
      ...
      for (int32_t i=0; i<paletteSize; ++i)
         palette[i] = os::Byteswap::byteswap(palette[i]);
      #endif
      }
      ...
      std::vector< uint8_t > data( dataBytes, 0x00 );
      file.seek( dataOffset );
      file.read( reinterpret_cast< char* >( data.data() ), dataBytes );
      file.close();
      ...
      //void * pDst = img.data();
      //converter( pSrc, pDst, size_t( w ) * h );
      convert8BitTo24Bit( data.data(), img.data(), w, h, palette, 0, false );
      img.flipVertical();
      return true;
   }
   else
