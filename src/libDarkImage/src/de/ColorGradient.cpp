...
LinearColorGradient::lerpColor128( uint32_t A, uint32_t B, float t )
{
   ...
   float const r = Math::clampf( startR + t * deltaR, 0.0f, 1.0f );
   float const g = Math::clampf( startG + t * deltaG, 0.0f, 1.0f );
   float const b = Math::clampf( startB + t * deltaB, 0.0f, 1.0f );
   float const a = Math::clampf( startA + t * deltaA, 0.0f, 1.0f );
...
LinearColorGradient::getColor128( float t ) const
{
   ...
   else
   {
      ...
      {
         if ( Math::equals( t, m_Stops[ i ].stop ) )
         {
...
//   m_Stops[ index ].stop = Math::clampf( t, 0.0f, 1.0f );
//   return *this;
//}
