...
ImagePainter::brighten( Image & img, float factor )
{
   for ( int32_t y = 0; y < img.getHeight(); ++y )
   {
      for ( int32_t x = 0; x < img.getWidth(); ++x )
      {
         ...
         int r = Math::clampi( Math::round32( factor * float( RGBA_R( color ) ) ), 0, 255 );
         int g = Math::clampi( Math::round32( factor * float( RGBA_G( color ) ) ), 0, 255 );
         int b = Math::clampi( Math::round32( factor * float( RGBA_B( color ) ) ), 0, 255 );
...
ImagePainter::autoSaturate( Image & img )
{
   ...
   for ( int32_t y = 0; y < img.getHeight(); ++y )
   {
      ...
      {
         uint32_t color = img.getPixel( x,y );
         int r = Math::clampi( (255 * (RGBA_Ri( color ) - r1)) / (r2-r1), 0, 255 );
         int g = Math::clampi( (255 * (RGBA_Gi( color ) - g1)) / (g2-g1), 0, 255 );
         int b = Math::clampi( (255 * (RGBA_Bi( color ) - b1)) / (b2-b1), 0, 255 );
         // int a = Math::clampi( ( 255 * ( RGBA_A( color ) - sA )) / dA, 0, 255 );
         img.setPixel( x,y, RGBA( r,g,b,RGBA_A( color ) ) );
      }
   }
}
...
ImageRotate::rotate( Image const & img, float angleDegreesCCW, uint32_t colorKey )
{
   ...
   //
   if ( Math::equals( angleDegreesCCW, 0.0f ) )
   {
      return img;
   }
   else if ( Math::equals( angleDegreesCCW, 90.0f))
   {
      return rotateLeft( img );
   }
   else if ( Math::equals( angleDegreesCCW, 270.0f))
   {
      return rotateRight( img );
   }
   else if ( Math::equals( angleDegreesCCW, 180.0f))
   {
      Image tmp = img;
      rotate180( tmp );
      return tmp;
...
//      if (Dnx>maxX) maxX=Dnx; if (Dny>maxY) maxY=Dny;
      ...
      int32_t dx = Math::round32( fdx );
      int32_t dy = Math::round32( fdy );
      ...
      for ( int32_t j = 0; j < dy; ++j )
      {
         for ( int32_t i = 0; i < dx; ++i )
         {
            if ( ( i < dst.getWidth() ) && ( j < dst.getHeight() ) )
            {
               ...
               int32_t x = Math::round32( x1 );
               int32_t y = Math::round32( y1 );
...
{
   int32_t const i = Math::round32( fx );
   int32_t const j = Math::round32( fy );
...
ImageScaler::getBilinearPixel( IImage* src, float fx, float fy, E_COLOR_PLANE planes, uint32_t* colorKey)
{
   ...
   uint32_t const & D=src.getPixel( i,   j+1); // read pixel, even if out of bounds (gets most nearest pixel)
    ...
    {   float fa = A.getAlpha()*e*f+B.getAlpha()*x*f+C.getAlpha()*x*y+D.getAlpha()*e*y;
        a = (uint32_t)core::s32_clamp( (s32)Math::round32(fa), 0, 255);
    }
    if (planes & ECP_RED)
    {   float fr = A.getRed()*e*f+B.getRed()*x*f+C.getRed()*x*y+D.getRed()*e*y;
        r = (uint32_t)core::s32_clamp( (s32)Math::round32(fr), 0, 255);
    }
    if (planes & ECP_GREEN)
    {   float fg = A.getGreen()*e*f+B.getGreen()*x*f+C.getGreen()*x*y+D.getGreen()*e*y;
        g = (uint32_t)core::s32_clamp( (s32)Math::round32(fg), 0, 255);
    }
    if (planes & ECP_BLUE)
    {   float fb = A.getBlue()*e*f+B.getBlue()*x*f+C.getBlue()*x*y+D.getBlue()*e*y;
        b = (uint32_t)core::s32_clamp( (s32)Math::round32(fb), 0, 255);
    }
...
//   {
      ...
      iA = Math::clampi( Math::round32( fa ), 0, 255 );
//   }
...
//   {
      ...
      iR = Math::clampi( Math::round32( fr ), 0, 255 );
//   }
...
//   {
      ...
      iG = Math::clampi( Math::round32( fg ), 0, 255 );
//   }
...
//   {
      ...
      iB = Math::clampi( Math::round32( fb ), 0, 255 );
//   }
...
{
   Image dst( region.getWidth(), region.getHeight(), src.getFormat() );
   dst.fill( clearColor );
